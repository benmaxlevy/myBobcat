                            ...
                            {((new Date()).getDay() === 0 || (new Date()).getDay() === 6) ? (
                                ...
                                (day === "") ? (
                                    ...
                                    <Text fontSize={"lg"} style={{textAlign: "center", color: "white"}}>
                                        ...
                                        {(await getPeriod(day) === -1) ? (
                                            <Text bold> no period right now</Text>
                                        ) : (
                                            <> period <Text
                                                bold>{(await getPeriod(day))[0]} {((await getPeriod(day))[2] !== "") ? (<>({(await getPeriod(day))[2]})</>) : (<></>)}</Text> with <Text
                                                bold>{(await getPeriod(day))[1]} minutes left</Text></>
                                        )}
                                        !</Text>
                                )
                            )
